            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/tzinfoList" method="post" class="form-inline">
							<input class="input-xlarge" placeholder="标题或内容..." id="key" name="key" type="text" value="">
		                    <select id="key1" name="fid" onChange="Change_Select()" class="input-xlarge">
							<option value="">请选择大类</option>
							<c:forEach items="${ctlist}" var="ftype">
					    	<option value="${ftype.id}" ${ftype.id eq fid ?"selected":""}>${ftype.typename}</option>
					    	</c:forEach>
							</select>
							<select id="key2" name="sid" class="input-xlarge">
							<option value="">请选择</option>
                                <c:forEach items="${childlist}" var="ctype">
                                    <option value="${ctype.id}" ${ctype.id eq sid ?"selected":""}>${ctype.typename}</option>
                                </c:forEach>
						    </select>
    ...
    <table class="table">
      ...
      <c:forEach items="${pageInfo.list}" var="tzinfo">
        ...
        <tr>
          ...
          <td>
              <a href="admin/findBbstype?id=${tzinfo.id}">迁移</a>
              <c:choose>
              <c:when test="${tzinfo.isjh eq 'no'}">
              <a href="admin/tzjjAdmin?id=${tzinfo.id}">精华</a>
              </c:when>
              <c:otherwise>
              <a href="admin/tzjjAdmin?id=${tzinfo.id}">取消精华</a>
              </c:otherwise>
              </c:choose>
              
              <c:choose>
              <c:when test="${tzinfo.istop eq 'no'}">
              <a href="admin/tzzdAdmin?id=${tzinfo.id}">置顶</a>
              </c:when>
              <c:otherwise>
              <a href="admin/tzzdAdmin?id=${tzinfo.id}">取消置顶</a>
              </c:otherwise>
              </c:choose>
              <a href="admin/tzinfoDel?id=${tzinfo.id}" ><i class="icon-remove"></i></a>
          </td>
        </tr>
        </c:forEach>
      <tr >
          <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="9">
              <c:if test="${fn:length(pageInfo.list) gt 0}">
                  <div id="pagediv" style="border:0px solid red;">
                      <ul>
                          <li>
                              <a href="admin/tzinfoList?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}&key=${key}">上一页</a>
                          </li>
                          <c:forEach begin="1" end="${pageInfo.pages}"
                                     varStatus="status">
                              <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}">
                                  <a href="admin/tzinfoList?pageNum=${status.count}&key=${key}">${status.count}</a>
                              </li>
                          </c:forEach>
                          <li>
                              <a href="admin/tzinfoList?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}&key=${key}">下一页</a>
                          </li>
                      </ul>
                  </div>
              </c:if>
