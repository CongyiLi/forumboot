            ...
            <div class="row-fluid">
				<div class="btn-toolbar">
					<div class="search-well">
						<form action="admin/msgList" method="post" class="form-inline">
							<input class="input-xlarge" placeholder="留言人..." value="${key}" id="key" name="key" type="text">
    ...
    <table class="table">
      ...
      <c:forEach items="${pageInfo.list}" var="chat">
        <tr>
          ...
          <td>${chat.savetime }</td>
          <td>
              <a href="admin/msgedit.jsp?id=${chat.id}"><i class="icon-pencil"></i></a>
              <a href="admin/chatDel?id=${chat.id}" ><i class="icon-remove"></i></a>
          </td>
        </tr>
       </c:forEach>
       <tr >
           <td style="font-weight: bold;font-family:楷体;font-weight: bold; color:blue;text-align: right;" colspan="9">
               <c:if test="${fn:length(pageInfo.list) gt 0}">
                   <div id="pagediv" style="border:0px solid red;">
                       <ul>
                           <li>
                               <a href="admin/msgList?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}&key=${key}">上一页</a>
                           </li>
                           <c:forEach begin="1" end="${pageInfo.pages}"
                                      varStatus="status">
                               <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}">
                                   <a href="admin/msgList?pageNum=${status.count}&key=${key}">${status.count}</a>
                               </li>
                           </c:forEach>
                           <li>
                               <a href="admin/msgList?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}&key=${key}">下一页</a>
                           </li>
                       </ul>
                   </div>
               </c:if>
